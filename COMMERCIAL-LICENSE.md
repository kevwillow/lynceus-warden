...
and — this is the part AGPL adds over plain GPL — that includes anyone who
only ever interacts with it *over a network*. Run a modified Lynceus as a
service and your users are entitled to that modified source. See
...
- **It does not change the dependencies.** Lynceus's runtime dependency closure
  is MIT, BSD-3-Clause, Apache-2.0, MPL-2.0 and PSF-2.0 — all compatible with
  AGPL-3.0, none copyleft-incompatible. The vendored Pico CSS keeps its own MIT
  licence.
