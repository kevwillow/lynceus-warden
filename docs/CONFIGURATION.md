...
| `ui_allowlist_path` | string \| null | `null` | Where the daemon writes the suppressions made from the web UI ("allowlist this device", "snooze"). When unset it is derived: the file sits in the STATE directory beside `db_path`, not beside `allowlist_path`. It is daemon-written state rather than operator config, and on a `--system` install the config directory is deliberately not writable by the daemon. An install upgrading from a version that wrote it beside `allowlist_path` needs no action: that file is read until the first UI suppression, which copies its entries across. | `/var/lib/lynceus/allowlist_ui.yaml` |
...
| `heartbeat_enabled` | boolean | `false` | Periodic "still watching" push, so that silence becomes meaningful. Without it, a dead daemon and a quiet environment look identical to the operator. Requires ntfy. | `true` |
| `heartbeat_interval_hours` | integer | `24` | How often the heartbeat is delivered. The clock runs from the last *delivered* heartbeat, not the last attempted one. Minimum `1`. | `12` |
