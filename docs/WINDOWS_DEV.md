...
Use [config/lynceus.dev.example.yaml](../config/lynceus.dev.example.yaml) as a template. Copy it to `lynceus.yaml` in the repo root:

```powershell
Copy-Item config/lynceus.dev.example.yaml lynceus.yaml
```
...
```powershell
lynceus-ui --config lynceus.yaml
```
...
- The systemd unit files in `deploy/` are Linux-specific; ignore them on Windows. The `lynceus` and `lynceus-ui` console scripts are the dev path.
