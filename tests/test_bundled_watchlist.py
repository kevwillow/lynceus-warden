...
def test_bundled_csv_ssid_rows_land_in_watchlist_db(tmp_path):
    ...
    try:
        ...
        # Mechanism-only assertions: the bundled snapshot drifts as
        # Argus accumulates new rows, so we pin the Flock-camera
        # operational contract (which the SSID activation work
        # depends on) without locking the test to specific row
        # counts. v0.7.6 Tier 4's snapshot refresh broke the prior
        # exact-equality pins (4 ssid + 24 ssid_pattern rows post-
        # refresh, vs. 3 + 5 pre-refresh); future refreshes will
        # drift again.
        ssid_patterns = sorted(r["pattern"] for r in ssid_rows)
        assert "Flock" in ssid_patterns, (
            f"Flock must appear as a bundled ssid row; got {ssid_patterns}"
        )
        sp_patterns = sorted(r["pattern"] for r in ssid_pattern_rows)
        # Flock case variants are a stable signal of the bundled
        # SSID activation surface; any drop here would silently
        # disable the argus_ssid alerting promise for fresh installs.
        assert any(p.lower() == "flock" for p in sp_patterns), (
            f"a Flock ssid_pattern row must appear in the bundle; "
            f"got {sp_patterns}"
        )
    finally:
        db.close()
...
def test_bundled_csv_end_to_end_flock_observation_fires_argus_ssid_alert(tmp_path):
    ...
    try:
        ...
        # rule literal. The bundled CSV's two Flock ssid_exact rows
        # (argus_record_id 69248a5dad0c2eab, cat=gunshot_detect and
        # cat=alpr, both at conf=85) hit the within-import dup gate.
        # With conf=85 >= the confidence-downgrade threshold (70),
        # neither peer is downgraded, so highest-severity-wins resolves
        # Flock at its native `high`. (Before the 2026-06-03 Argus
        # refresh the bundled Flock rows were conf=65, which downgraded
        # alpr high->med and gunshot_detect med->low, landing the alert
        # at `med`; the refresh raised Flock's confidence to 85, lifting
        # it to `high` out of the box.) This test pins both: (1) the
        # alpr/high outcome explicitly, and (2) DB-delegation by
        # comparing alert severity to the stored watchlist row's severity.
        ...
        assert stored["severity"] == "high", (
            f"highest-severity-wins must resolve Flock at `high` (alpr "
            f"peer at conf=85, no downgrade); got "
