...
def test_migrations_dir_lists_both_files(db):
    ...
    _EXPECTED_MIGRATIONS = [
        ...
        "028_watchful_one_active_per_mac.sql",
    ]
...
def test_list_devices_includes_ble_name_and_probe_ssids(db):
    """v0.7.3: list_devices SELECT widened to cover Kismet-sourced
    forensic columns so the /devices and home-page tables can render
    BLE friendly names + probed SSIDs without a second query."""
    mac = "aa:bb:cc:dd:ee:42"
    db.upsert_device(mac, "ble", "Sony Corp", 0, 100)
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["ble_name"] == "Sony WH-1000XM4"
    # probe_ssids round-trips through json.dumps in storage; the SELECT
    # returns the raw JSON string -- decoding happens in the template
    # filter layer.
    assert row["probe_ssids"] is not None
    assert "HomeNet" in row["probe_ssids"]
    assert "OfficeWiFi" in row["probe_ssids"]


def test_list_devices_returns_none_for_missing_ble_name_and_probes(db):
    """Devices without a BLE name or any probe history surface those
    keys as None -- the template filter chain renders the em-dash
    fallback for ble_name and an empty cell for probes."""
    db.upsert_device("aa:bb:cc:dd:ee:01", "wifi", "Acme", 0, 100)
    rows = db.list_devices()
    assert len(rows) == 1
    assert rows[0]["ble_name"] is None
    assert rows[0]["probe_ssids"] is None


def _seed_mixed_devices(db):
    """Seed one device of each type plus a probing/non-probing split.

    Returns nothing; callers assert against list/count_devices. The
    wifi device with merged probe SSIDs is the only "probing" row; the
    second wifi device stays probe-less (NULL probe_ssids).
    """
    db.upsert_device("aa:bb:cc:dd:ee:01", "wifi", "Acme", 0, 100)
    db.merge_device_probe_ssids("aa:bb:cc:dd:ee:01", ["HomeNet"])
    db.upsert_device("aa:bb:cc:dd:ee:02", "wifi", "Acme", 0, 110)
    db.upsert_device("aa:bb:cc:dd:ee:03", "ble", "Sony", 0, 120)
    db.upsert_device("aa:bb:cc:dd:ee:04", "bt_classic", "Bose", 0, 130)
    db.upsert_device("aa:bb:cc:dd:ee:05", "remote_id", None, 0, 140)


def test_list_devices_probing_filter(db):
    """v0.8.0 Touch 1: probing=True returns only devices with a
    non-empty stored probe_ssids; probing=False returns the rest;
    probing=None (default) returns everything. The "no probes" stored
    value is NULL, so the False bucket must catch NULL rows."""
    _seed_mixed_devices(db)
    probing = {r["mac"] for r in db.list_devices(probing=True)}
    assert probing == {"aa:bb:cc:dd:ee:01"}
    not_probing = {r["mac"] for r in db.list_devices(probing=False)}
    assert "aa:bb:cc:dd:ee:01" not in not_probing
    assert "aa:bb:cc:dd:ee:02" in not_probing  # NULL probe_ssids lands here
    assert len(db.list_devices(probing=None)) == 5
    # COUNT(*) stays consistent with the page query under the filter.
    assert db.count_devices(probing=True) == 1
    assert db.count_devices(probing=False) == 4
    assert db.count_devices(probing=None) == 5


def test_list_devices_bluetooth_alias_expands_to_both_subtypes(db):
    """v0.8.0 Touch 1: device_type='bluetooth' is a query-only alias
    expanding to ('ble','bt_classic') -- never a stored value. It must
    return both BT subtypes and exclude wifi / remote_id, and the
    COUNT(*) must agree."""
    _seed_mixed_devices(db)
    bt = {r["device_type"] for r in db.list_devices(device_type="bluetooth")}
    assert bt == {"ble", "bt_classic"}
    assert db.count_devices(device_type="bluetooth") == 2
    # The individual subtypes still filter exactly as before.
    assert db.count_devices(device_type="ble") == 1
    assert db.count_devices(device_type="bt_classic") == 1
    # remote_id is filterable; bogus literals still raise.
    assert db.count_devices(device_type="remote_id") == 1
    with pytest.raises(ValueError):
        db.list_devices(device_type="zigbee")
    with pytest.raises(ValueError):
        db.count_devices(device_type="zigbee")


def test_list_devices_probing_composes_with_type_and_randomized(db):
    """v0.8.0 Touch 1: the new probing dimension AND-combines with the
    existing device_type / randomized filters rather than replacing
    them, and list/count remain consistent under the combination."""
    db.upsert_device("aa:bb:cc:dd:ee:10", "wifi", "Acme", 1, 200)
    db.merge_device_probe_ssids("aa:bb:cc:dd:ee:10", ["RandProbe"])
    db.upsert_device("aa:bb:cc:dd:ee:11", "wifi", "Acme", 0, 210)
    db.merge_device_probe_ssids("aa:bb:cc:dd:ee:11", ["StableProbe"])
    rows = db.list_devices(device_type="wifi", randomized=True, probing=True)
    assert {r["mac"] for r in rows} == {"aa:bb:cc:dd:ee:10"}
    assert db.count_devices(device_type="wifi", randomized=True, probing=True) == 1
    assert db.count_devices(device_type="wifi", randomized=False, probing=True) == 1


def test_list_devices_surfaces_most_recent_rssi_and_ssid(db):
    """Touchup arc: the devices list shows last_rssi / last_ssid drawn
    from the MOST RECENT sighting per device, not an arbitrary row.
    Insert three sightings with distinct timestamps and confirm the
    SELECT returns the latest. Without the per-mac DESC ordering this
    would surface whichever row sqlite happened to read first."""
    mac = "aa:bb:cc:dd:ee:01"
    db.ensure_location(LOC, "Lab")
    db.upsert_device(mac, "wifi", "Acme", 0, 300)
    db.insert_sighting(mac, 100, -80, "OldNet", LOC)
    db.insert_sighting(mac, 300, -55, "NewNet", LOC)
    db.insert_sighting(mac, 200, -70, "MidNet", LOC)
    rows = db.list_devices()
    assert len(rows) == 1
    assert rows[0]["last_rssi"] == -55
    assert rows[0]["last_ssid"] == "NewNet"


def test_list_devices_renders_none_for_device_without_sightings(db):
    """Devices upserted but never sighted (poller crashed between
    upsert_device and insert_sighting, or test fixtures that skip the
    sighting step) surface last_rssi / last_ssid as None — the LEFT-
    JOIN-shaped correlated subquery returns NULL rather than raising,
    and the template renders an em-dash."""
    db.upsert_device("aa:bb:cc:dd:ee:01", "wifi", "Acme", 0, 100)
    rows = db.list_devices()
    assert len(rows) == 1
    assert rows[0]["last_rssi"] is None
    assert rows[0]["last_ssid"] is None


def test_list_devices_probe_only_wifi_renders_rssi_but_null_ssid(db):
    """A Wi-Fi device sighted only via probe requests (no association)
    has rssi populated but no associated SSID — sightings.ssid is NULL.
    The dict surface should reflect that: last_rssi == int, last_ssid
    is None."""
    mac = "aa:bb:cc:dd:ee:02"
    db.ensure_location(LOC, "Lab")
    db.upsert_device(mac, "wifi", "Acme", 0, 100)
    db.insert_sighting(mac, 100, -67, None, LOC)
    rows = db.list_devices()
    assert len(rows) == 1
    assert rows[0]["last_rssi"] == -67
    assert rows[0]["last_ssid"] is None
...
def test_migration_014_preserves_existing_rows_through_rebuild(tmp_path):
    ...
    try:
        ...
        # 23 is rewound alongside 14: a DB last opened by a pre-rc5 build
        # would carry neither, and 014's rebuild recreates devices at its
        # own era's column set — so leaving 23 marked applied would strand
        # the table without ble_device_class while schema_migrations
        # claimed otherwise.
        raw.execute("DELETE FROM schema_migrations WHERE version IN (14, 23)")
        raw.executescript(
            """
    ...
    try:
        ...
        with pytest.raises(sqlite3.IntegrityError):
            db._conn.execute(
                ...
                "VALUES (?, ?, ?, ?, 1, ?, ?)",
                ("aa:bb:cc:dd:ee:03", "zigbee", 1200, 1200, None, 0),
            )
    finally:
        db.close()
