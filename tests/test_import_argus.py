...
# ---- placeholder OUI skip (v0.7.9 Touch 7) -------------------------------


def test_placeholder_oui_row_skipped_with_log(tmp_path, db, caplog):
    """Argus emits identifier=00:00:00 + identifier_type=oui as a
    placeholder for vendor rows where no real OUI is known. The
    bundled snapshot's CCTV-vendor block holds ~40 such rows;
    importing them would clutter the watchlist DB with patterns
    that can never produce a meaningful match (and the rules-engine
    reserved-OUI guard already drops the match-time false positive
    in any case). Assert the row is dropped, counted in the
    dedicated bucket, and logged at INFO."""
    path = _write_csv(
        tmp_path / "wl.csv",
        [
            _row(
                argus_record_id="placeholder-1",
                identifier="00:00:00",
                identifier_type="oui",
    ...
    assert report.dropped_placeholder_oui == 1
    assert _wl_count(db) == 0
    info_lines = [
        r.getMessage() for r in caplog.records
        if r.levelno == _logging.INFO
        and "skipping placeholder OUI row" in r.getMessage()
    ]
    assert len(info_lines) == 1
    assert "argus_record_id=placeholder-1" in info_lines[0]


def test_placeholder_oui_skip_does_not_affect_legitimate_oui_rows(tmp_path, db):
    """Regression: a row whose identifier_type is oui but whose
    pattern is a real vendor prefix must STILL be admitted. The
    placeholder filter targets the literal 00:00:00 pattern only."""
    path = _write_csv(
        tmp_path / "wl.csv",
        [
            _row(
                argus_record_id="real-oui",
                identifier="aa:bb:cc",
                identifier_type="oui",
                device_category="cctv_camera",
                manufacturer="Acme",
    ...
    report = import_csv(db, path, OverrideConfig())
    assert report.imported_new == 1
    assert report.dropped_placeholder_oui == 0
    row = db._conn.execute(
        "SELECT pattern, pattern_type FROM watchlist"
    ).fetchone()
    assert row["pattern"] == "aa:bb:cc"
    assert row["pattern_type"] == "oui"


def test_placeholder_oui_skip_mixed_batch_admits_real_only(tmp_path, db):
    """A CSV with one placeholder and one legitimate OUI row must
    admit exactly the legitimate row and bucket the placeholder
    into dropped_placeholder_oui. The bundled snapshot ships ~40
    placeholder rows alongside thousands of legitimate ones; this
    mirrors that shape in miniature."""
    path = _write_csv(
        tmp_path / "wl.csv",
        [
            _row(
                argus_record_id="ph-1",
                identifier="00:00:00",
                identifier_type="oui",
                device_category="cctv_camera",
                manufacturer="Tiandy",
    ...
    assert len(rows) == 1
    assert rows[0]["pattern"] == "dc:a6:32"


def test_report_render_lists_placeholder_oui_bucket(tmp_path, db):
    """Operator-visible run summary surfaces the new bucket so a drop
    is auditable from the import report without grepping logs."""
    path = _write_csv(
        tmp_path / "wl.csv",
        [
            _row(
    ...
    assert "Dropped (placeholder_oui): 1" in rendered
    assert "placeholder_oui" in rendered  # short-form line
...
def test_operator_seeded_row_without_md_is_preserved_not_adopted(tmp_path, db):
    """G4: a YAML-seeded watchlist row with NO Argus metadata is
    operator-seeded — on a natural-key collision the operator OVERRIDES
    Argus. The incoming Argus row must NOT overwrite the operator's
    severity/description and must NOT attach metadata (signal integrity:
    attaching it would flip the row out of the operator-seeded branch
    and silently re-enable the overwrite on the next import). The decline
    is counted in ``operator_preserved``, not ``imported_new``. This
    reverses the pre-G4 adopt-on-first-contact behavior. See
    tests/test_diag_g4_severity_overwrite.py."""
    ...
    path = _write_csv(
        tmp_path / "wl.csv",
        [
            # device_category 'alpr' -> Argus severity 'high', disagreeing
            # with the operator's 'low'. The operator value must win.
    ...
    assert _md_count(db) == 0  # NOT adopted — no Argus metadata attached
    wl = db._conn.execute(
        "SELECT severity, description FROM watchlist WHERE pattern = ?",
        ("aa:bb:cc:dd:ee:ff",),
    ).fetchone()
    assert wl["severity"] == "low"            # operator value preserved
    assert wl["description"] == "yaml-seeded"  # operator value preserved
...
def _drop_log_records(
    caplog, argus_record_id: str, reason: str, level: int = _logging.INFO
) -> list:
    """Drop-log records at exactly ``level`` (default INFO).

    ``level`` exists because the unknown_type drop log is level-split:
    novel types stay at INFO for operator traceability, types already
    recorded as non-RF-observable drop to DEBUG so the bundled snapshot
    does not emit ~18k INFO lines per import.
    """
    return [
        r
        for r in caplog.records
...
def test_ble_characteristic_drop_logs_at_debug_not_info(tmp_path, db, caplog):
    """ble_characteristic drops, and does so quietly.

    Was pinned at INFO when the type read as "Lynceus does not yet
    support". It is now recorded in NON_RF_IDENTIFIER_TYPES as a
    permanent exclusion rather than a gap: a GATT characteristic UUID is
    only readable after CONNECTING to the device, and Lynceus is
    passive-only and never connects. So there is no future matcher to
    wait for, and the drop belongs with the other accounted-for types
    that collapse into the aggregate line.

    The per-row line still exists at DEBUG, so `--log-level DEBUG` keeps
    the old per-row traceability. Novel (unaccounted-for) types stay at
    INFO — see test_garbage_identifier_type_drop_emits_info_log."""
    path = _write_csv(
        tmp_path / "wl.csv",
    ...
    with caplog.at_level(_logging.INFO, logger="lynceus.cli.import_argus"):
        import_csv(db, path, OverrideConfig())
    assert _drop_log_records(caplog, "bc-logged", "unknown_identifier_type") == []

    caplog.clear()
    with caplog.at_level(_logging.DEBUG, logger="lynceus.cli.import_argus"):
        import_csv(db, path, OverrideConfig())
    matches = _drop_log_records(
        caplog, "bc-logged", "unknown_identifier_type", level=_logging.DEBUG
    )
    assert len(matches) == 1
...
def test_run_summary_line_formatted_correctly(tmp_path, db):
    ...
    report = import_csv(db, path, OverrideConfig())
    text = report.render()
    assert "imported 8 records, updated 0, operator-preserved 0, dropped 1" in text
...
def test_argus_schema_version_28_accepted_silently(tmp_path, db, caplog):
    """v0.7.7 Touch 7: extended the accept-list ceiling to 30 to cover
    schema versions 28-30 inclusively. v28 imports silently now."""
    path = _write_csv_with_meta(
        tmp_path / "v28.csv",
...
def test_argus_schema_version_29_accepted_silently(tmp_path, db, caplog):
    """v0.7.7 Touch 7: schema_version=29 sits inside the extended
    accept-list (25-31 inclusive)."""
    path = _write_csv_with_meta(
        tmp_path / "v29.csv",
        "# meta: schema_version=29, exported_at=2026-05-07T20:17:59Z, record_count=1",
        [_row(argus_record_id="v29-1")],
    )
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert _warnings_for_schema_version(caplog) == []
...
def test_argus_schema_version_30_accepted_silently(tmp_path, db, caplog):
    """v0.7.7 Touch 7: the refreshed bundled CSV from v0.7.6 Tier 4
    declares schema_version=30. Without the extended accept-list, the
    importer WARNed on every bundled import — exactly the symptom
    Touch 7 fixes."""
    path = _write_csv_with_meta(
        tmp_path / "v30.csv",
        "# meta: schema_version=30, exported_at=2026-05-25T00:00:00Z, record_count=1",
        [_row(argus_record_id="v30-1")],
    )
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert _warnings_for_schema_version(caplog) == []
...
def test_argus_schema_version_31_accepted_silently(tmp_path, db, caplog):
    """Post-0.9.1: the 2026-06-03 bundled CSV refresh declares
    schema_version=31, so 31 was added to the accept-list — it now
    imports silently (mirrors the schema_version=30 case)."""
    path = _write_csv_with_meta(
        tmp_path / "v31.csv",
        "# meta: schema_version=31, exported_at=2026-06-03T00:00:00Z, record_count=1",
        [_row(argus_record_id="v31-1")],
    )
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert _warnings_for_schema_version(caplog) == []
...
def test_argus_schema_version_24_below_floor_warns(tmp_path, db, caplog):
    """v0.7.7 Touch 7: the floor stays at 25 — v24 still WARNs even
    after extending the ceiling. Touch 7 widened, didn't shift, the
    accept range."""
    path = _write_csv_with_meta(
        tmp_path / "v24.csv",
        "# meta: schema_version=24, exported_at=2026-05-07T20:17:59Z, record_count=1",
        [_row(argus_record_id="v24-1")],
    )
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    warnings = _warnings_for_schema_version(caplog)
...
# ---------------------------------------------------------------------------
# unknown_type drop reporting (v0.9.5).
#
# The bundled Argus snapshot drops ~43% of its rows as unknown_type,
# because Argus carries intelligence Lynceus cannot observe over passive
# RF (hostnames, firmware strings, FCC codes). Two things follow: the
# per-row log must not run to five figures, and a NEW type that is not
# accounted for must be visible rather than silently absorbed.
# ---------------------------------------------------------------------------


def test_unknown_type_rows_do_not_log_per_row_at_info(tmp_path, db, caplog):
    """Regression: per-row unknown_type logging was INFO, which put
    17,952 lines in the journal on every bundled import (weekly under
    lynceus-refresh.timer) and buried the peer_collision lines that
    actually need reading. The per-row line is DEBUG now; the aggregate
    replaces it at INFO."""
    rows = [
        _row(argus_record_id=f"h{i}", identifier=f"cam{i}.example.net",
             identifier_type="vendor_controlled_hostname")
        for i in range(25)
    ]
    path = _write_csv(tmp_path / "hostnames.csv", rows)
    with caplog.at_level(_logging.INFO, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert report.dropped_unknown_type == 25
    per_row = [r for r in caplog.records if "unknown_identifier_type" in r.getMessage()]
    assert per_row == [], f"expected no per-row INFO lines, got {len(per_row)}"


def test_unknown_type_emits_one_aggregate_line_with_breakdown(tmp_path, db, caplog):
    """The replacement for the per-row flood: one INFO line naming each
    dropped type and its count, so the operator can tell 'expected non-RF
    split' from 'something broke'."""
    rows = [
        _row(argus_record_id="h1", identifier="a.example.net",
             identifier_type="vendor_controlled_hostname"),
        _row(argus_record_id="h2", identifier="b.example.net",
             identifier_type="vendor_controlled_hostname"),
        _row(argus_record_id="f1", identifier="ABC", identifier_type="fcc_grantee_code"),
    ]
    path = _write_csv(tmp_path / "mixed.csv", rows)
    with caplog.at_level(_logging.INFO, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert report.dropped_unknown_type == 3
    assert report.dropped_unknown_type_breakdown["vendor_controlled_hostname"] == 2
    assert report.dropped_unknown_type_breakdown["fcc_grantee_code"] == 1
    aggregates = [
        r.getMessage() for r in caplog.records if "no Lynceus matcher" in r.getMessage()
    ]
    assert len(aggregates) == 1, f"expected exactly one aggregate line, got {aggregates}"
    assert "vendor_controlled_hostname=2" in aggregates[0]
    assert "fcc_grantee_code=1" in aggregates[0]


def test_known_non_rf_types_do_not_warn(tmp_path, db, caplog):
    """Dropping a documented non-RF type is expected behavior, not a
    problem to page about. Only the unaccounted-for case warns."""
    rows = [
        _row(argus_record_id="h1", identifier="a.example.net",
             identifier_type="vendor_controlled_hostname"),
    ]
    path = _write_csv(tmp_path / "known.csv", rows)
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert report.dropped_unknown_type == 1
...
def test_unrecognized_dropped_type_warns_as_possible_missing_matcher(tmp_path, db, caplog):
    """The capability-regression guard. An Argus release that adds an
    RF-observable identifier type Lynceus has no matcher for would
    otherwise vanish into the same silent bucket as the hostnames and
    quietly shrink detection coverage. It has to be loud."""
    rows = [
        _row(argus_record_id="new1", identifier="beacon-xyz",
             identifier_type="ble_extended_adv_pattern"),
    ]
    path = _write_csv(tmp_path / "novel.csv", rows)
    with caplog.at_level(_logging.WARNING, logger="lynceus.cli.import_argus"):
        report = import_csv(db, path, OverrideConfig())
    assert report.dropped_unknown_type == 1
    assert report.unrecognized_dropped_types() == ["ble_extended_adv_pattern"]
    warnings = [
        r.getMessage()
        for r in caplog.records
        if r.levelno >= _logging.WARNING and "neither mapped nor recorded" in r.getMessage()
    ]
    assert len(warnings) == 1, f"expected one missing-matcher warning, got {warnings}"
    assert "ble_extended_adv_pattern=1" in warnings[0]


def test_render_gloss_explains_unknown_type_and_is_ascii(tmp_path, db):
    """``Dropped (unknown_type): 17952`` alone reads as breakage. The
    rendered line says why and names the top contributors. ASCII only —
    render() goes through print() to a Windows console on dev hosts."""
    rows = [
        _row(argus_record_id="h1", identifier="a.example.net",
             identifier_type="vendor_controlled_hostname"),
        _row(argus_record_id="new1", identifier="zzz",
             identifier_type="totally_novel_type"),
    ]
    path = _write_csv(tmp_path / "gloss.csv", rows)
    report = import_csv(db, path, OverrideConfig())
    rendered = report.render()
    assert "no Lynceus matcher" in rendered
    assert "vendor_controlled_hostname 1" in rendered
    assert "UNRECOGNIZED: totally_novel_type" in rendered
    rendered.encode("ascii")  # raises UnicodeEncodeError on a non-ASCII glyph


def test_non_rf_set_and_identifier_map_are_disjoint():
    """A type cannot be both 'we match this' and 'this is not
    RF-observable'. If a type is promoted to a real matcher, it must be
    removed from NON_RF_IDENTIFIER_TYPES in the same change."""
    overlap = set(IDENTIFIER_TYPE_MAP) & import_argus.NON_RF_IDENTIFIER_TYPES
    assert overlap == set(), f"types in both map and non-RF set: {sorted(overlap)}"


def test_bundled_snapshot_has_no_unrecognized_dropped_types(tmp_path, db):
    """Every identifier_type the shipped snapshot drops is accounted for
    — either mapped to a matcher or recorded as non-RF. A new Argus
    export that trips this is the signal to go look at what changed,
    which is exactly what the warning path is for."""
    from lynceus.data import __file__ as data_init
    bundled = Path(data_init).parent / "default_watchlist.csv"
    if not bundled.exists():  # pragma: no cover - packaging guard
