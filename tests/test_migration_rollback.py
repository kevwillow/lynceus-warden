...
def test_rollback_to_zero_then_reapply(db_path):
    """Full chain: apply 001..022, roll back to 0, re-apply 001..022.
...
def test_rollback_one_step_each(db_path):
    """For every applied version V from 22 down to 1, roll back ONE step
    and confirm:
...
def test_rollback_to_specific_target(db_path):
    """Forward-apply 001..023, roll back to 015 (so 016..023 revert),
