...
# ---- watchlist_oui reserved/bogus OUI filter (v0.7.9 Touch 6) -------------


@pytest.fixture
def db_with_reserved_oui_rows(tmp_path):
    """Seed the watchlist with rows whose patterns match the reserved/
    bogus prefixes the v0.7.9 filter is supposed to suppress, plus a
    legitimate vendor OUI for the regression case. Mirrors the shape
    of the bundled Argus snapshot: ~40 rows in default_watchlist.csv
    carry pattern=00:00:00 as a placeholder for CCTV vendors where no
    real OUI was known."""
    db_path = str(tmp_path / "rules_reserved_oui.db")
    db = Database(db_path)
    with db._conn:
        for pattern in ("00:00:00", "ff:ff:ff", "01:00:5e", "33:33:00"):
            db._conn.execute(
                "INSERT INTO watchlist(pattern, pattern_type, severity, description) "
                "VALUES (?, 'oui', 'high', ?)",
                (pattern, f"reserved OUI row {pattern}"),
            )
        # Also seed a locally-administered OUI as a watchlist row.
        db._conn.execute(
            "INSERT INTO watchlist(pattern, pattern_type, severity, description) "
            "VALUES ('02:42:ac', 'oui', 'high', 'locally-administered docker prefix')",
        )
        # Legitimate vendor OUI for the non-regression assertion.
        db._conn.execute(
            "INSERT INTO watchlist(pattern, pattern_type, severity, description) "
            "VALUES ('a4:83:e7', 'oui', 'med', 'legitimate vendor')",
        )
    yield db
    db.close()
...
@pytest.mark.parametrize(
    "mac,prefix_label",
    [
        ("00:00:00:00:00:00", "00:00:00 placeholder"),
        ("00:00:00:de:ad:be", "00:00:00 placeholder w/ trailing data"),
        ("ff:ff:ff:ff:ff:ff", "ff:ff:ff broadcast"),
        ("01:00:5e:01:02:03", "01:00:5e IPv4 multicast"),
        ("33:33:00:00:00:01", "33:33 IPv6 multicast"),
        ("02:42:ac:11:00:02", "locally-administered (02:42 docker)"),
        ("06:11:22:33:44:55", "locally-administered (06:* nibble)"),
        ("0a:bb:cc:dd:ee:ff", "locally-administered (0a:* nibble)"),
        ("0e:11:22:33:44:55", "locally-administered (0e:* nibble)"),
        ("12:34:56:78:9a:bc", "locally-administered (12:* high octet)"),
    ],
)
def test_evaluate_watchlist_oui_skips_reserved_or_bogus(
    ...
    carries the locally-administered bit, even when the watchlist
    table contains a matching pattern. These MACs categorically do
    not represent the device the operator wanted to be alerted on.
    Documents the prefix_label for readable failure output."""
    rule = Rule(name="argus_oui", rule_type="watchlist_oui", severity="low", patterns=[])
    rs = Ruleset(rules=[rule])
    with caplog.at_level(logging.DEBUG, logger="lynceus.rules"):
        hits = evaluate(
            rs,
            _obs(mac=mac),
            is_new_device=False,
...
def test_evaluate_watchlist_oui_still_matches_legitimate_vendor(
    db_with_reserved_oui_rows,
):
    """Regression: the new filter must not suppress legitimate vendor
    OUI hits. The fixture seeds a4:83:e7 (Apple-allocated IEEE OUI);
    a MAC under that prefix must still match the watchlist row."""
    rule = Rule(name="argus_oui", rule_type="watchlist_oui", severity="low", patterns=[])
    rs = Ruleset(rules=[rule])
    hits = evaluate(
        rs,
        _obs(mac="a4:83:e7:de:ad:be"),
        is_new_device=False,
        db=db_with_reserved_oui_rows,
    )
    assert len(hits) == 1
    assert hits[0].severity == "med"  # from DB row
    assert hits[0].mac == "a4:83:e7:de:ad:be"


def test_evaluate_watchlist_oui_zero_mac_against_empty_db_returns_none(tmp_path):
    """Regression: filtering 00:00:00 must short-circuit cleanly with
    no exception even when the watchlist is empty (the SQL lookup
    never runs because the reserved-OUI guard fires first)."""
...
def test_evaluate_watchlist_oui_in_memory_path_unaffected_by_filter():
    """Regression: the reserved-OUI filter sits on the delegation
    branch only. The in-memory match path (non-empty rule.patterns)
    is intentionally unchanged -- operators who hand-author a
    rule.yaml with patterns=['00:00:00'] are doing it on purpose and
    we shouldn't second-guess them."""
    rule = Rule(
        name="legacy_zero_oui",
        rule_type="watchlist_oui",
        severity="high",
        patterns=["00:00:00"],
    )
    rs = Ruleset(rules=[rule])
    hits = evaluate(rs, _obs(mac="00:00:00:de:ad:be"), is_new_device=False)
    assert len(hits) == 1
    assert hits[0].severity == "high"
