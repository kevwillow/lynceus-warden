...
def _wifi_raw_with_probes(mac: str, probes: list[str], last_time: int = 1700000100) -> dict:
    ...
    Mirrors Kismet's real emission: probed SSIDs live in a LIST of records
    at ``dot11.device.probed_ssid_map``, each record carrying the leaf
    ``dot11.probedssid.ssid``.
    """
    probed_ssid_map = [{"dot11.probedssid.ssid": probe} for probe in probes]
    return {
        ...
        "dot11.device": {
            "dot11.device.probed_ssid_map": probed_ssid_map,
        },
    }
...
def test_parse_probe_ssids_skipped_for_ble_devices():
    ...
    raw["dot11.device"] = {
        "dot11.device.probed_ssid_map": [
            {"dot11.probedssid.ssid": "ShouldNotBeRead"}
        ]
    }
...
def test_poller_probe_ssids_opt_out_does_not_read_kismet_field(db, db_path):
    ...
    assert "dot11.device" not in raw.accessed
    assert "dot11.device.probed_ssid_map" not in raw.accessed
    row = db._conn.execute(
