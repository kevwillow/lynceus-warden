...
@pytest.mark.webui
def test_watchful_actions_column_uses_flex_alignment(tmp_path):
    """v0.7.9 Touch 5: each Actions cell on /watchful mixes <form> and
    <details> siblings (dismiss + promote + investigate + confirmed-
    safe, plus reset on escalated rows). Without a flex container they
    stacked at inconsistent vertical positions because <details>
    carries a different baseline than the form buttons. Assert the
    template still emits the .watchful-actions wrapper and the CSS
    ships the flex declaration that aligns its children."""
    app, db, _ = _make_app(tmp_path)
    try:
        _insert_watchful(db)
        with TestClient(app) as client:
        ...
        assert r.status_code == 200
        # Template still renders the wrapper class the CSS targets.
        assert 'class="watchful-actions"' in r.text
        ...
        # Wrap so a narrow viewport flows actions onto a second row
        # rather than clipping past the cell edge.
