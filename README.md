...
- **Passive BLE bridge and Apple Continuity decoder (both off by default).** Kismet's classic Bluetooth datasource hands over no advertisement payload, which left the `ble_uuid`, `ble_manufacturer_id`, and drone Remote-ID matchers with nothing to chew on. A passive `bleak` scan on its own adapter now feeds them, and a decoder sorts Apple adverts by Continuity message type into `find_my`, `airpods`, `nearby`, or `apple_unknown`. Every Apple device shares company id `004c`, so matching the bare id would page you for each pair of AirPods that strolls by. The class lands on the device row, shows up on `/devices`, and drives an optional `ble_device_class` rule. Payload bytes get read inside the scan callback and dropped there. Only the label survives, and a regression test fails the build if anyone starts buffering the raw bytes.

  Where this actually stands: the bridge sleeps until you set `ble_bridge.enabled`, the rule ships commented out, and nobody has yet pointed the decoder at a real AirTag. Built and tested, not field-proven. Telling a stalking tracker from your own keys needs the separated-from-owner bit, and that one waits on a hardware capture.
