...
[![Status: v0.7.1](https://img.shields.io/badge/Status-v0.7.1-blue.svg)](#project-status)
...
**Personal-use RF security monitoring tool. v0.7.1.**

This is not a hardened public product. It is a personal project, feature-complete for v0.7.1, with Kali hardware-smoke validation pending. Use it on hardware you control, in a jurisdiction where passive RF observation is legal, and read the source before trusting it with anything.

Test count: **2823 passing on Windows / 2841 expected on Linux** at v0.7.1 (the 18-test delta is POSIX-only `install.sh` + `chmod`-mode tests that skip on non-POSIX hosts). Up from 2812 at v0.7.0, 2526 at v0.6.1, 2475 at v0.6.0, 2434 at v0.5.0, 1215 at v0.4.0-rc4, and 888 at v0.3.0-rc1. A separate diagnostic suite of **22 behavior-dump tests** (`pytest -m diagnostic`) is run pre-push for pre-flight observation and is excluded from the default suite.
