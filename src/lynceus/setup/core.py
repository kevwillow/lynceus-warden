...
# pass-3 commits per row (one `db.transaction()` block per survivor), so
...
def render_config_yaml(answers: dict) -> str:
    ...
    for src in answers["kismet_sources"]:
        sources_lines.append(f"  - {_yaml_str(src)}")
    lines = [
        ...
        f"kismet_url: {_yaml_str(answers['kismet_url'])}",
...
def carry_forward_settings(
    existing_path: Path, rendered: str
    ...
    ⛔ **Only settings the merged config will actually LOAD are carried forward**,
    and the rest are reported by name. ``Config`` sets ``extra="forbid"``, so
    carrying an unrecognised key forward verbatim makes the config fail to load —
    turning a typo into a daemon that will not start. Measured: an existing file
    holding ``heartbeat_interal_hours: 12`` (one transposed letter) survived
    ``--reconfigure`` and then raised ``ValidationError`` at load. That is a
    REGRESSION the carry-forward introduced: before it, the misspelling was
    discarded and the wizard produced a runnable config. Recovering from a typo
    by re-running setup is exactly what an operator would try, so preservation
    must not take that away.

    🪤 **A name check is not enough, and stopping there was the first fix's own
    bug.** ``heartbeat_interval_hours: nope`` is a perfectly good key with a
    value ``Config`` rejects, so filtering on ``Config.model_fields`` alone still
    produced a config the daemon could not load. The candidate document is
    therefore VALIDATED, and any carried key whose value pydantic rejects is
    dropped too. Both checks derive from ``Config`` itself, so a field added,
    renamed or re-validated there moves them with no edit here.

    ⚠️ A validation failure caused by the WIZARD's own answers is left alone — it
    is not the carried block's fault and silently dropping operator settings to
    paper over it would be the worse error.

    Returns ``(preserved, dropped, error)``. ``dropped`` maps each discarded key
    to why, so the caller can name it: an operator cannot fix a typo they are
    never shown.

    ⚠️ ``error`` is a REPORT, not a refusal: the caller still has to decide what
    to do with a file it could not read. See ``apply_config``, which backs the
    file up and refuses if it cannot.
    """
    ...
    candidates = {k: v for k, v in existing.items() if k not in rendered_keys}
    known = set(Config.model_fields)
    dropped: dict[str, str] = {}

    # 🪤 YAML keys are not necessarily strings: `123: value` is a legal mapping,
    # and an int key sorted or joined against str keys raised TypeError — AFTER
    # the config had been written, so the wizard crashed mid-apply having
    # already replaced the file. Report every key as text; a non-string key
    # cannot name a Config field anyway.
    preserved: dict = {}
    for key, value in candidates.items():
        if isinstance(key, str) and key in known:
            preserved[key] = value
        else:
            dropped[str(key)] = "not a Lynceus setting"

    # Then the value check, because a valid NAME can still carry a value that
    # stops the file loading. Bounded by construction: each pass drops at least
    # one key or stops.
    while preserved:
        try:
            Config(**{**rendered_keys, **preserved})
            break
        except ValidationError as exc:
            bad = {
                str(err["loc"][0]): err.get("msg", "invalid value")
                for err in exc.errors()
                if err.get("loc") and str(err["loc"][0]) in preserved
            }
            if not bad:
                # The failure is in the wizard's own answers, not in what we
                # carried. Not ours to paper over by discarding operator data.
                break
            for key, why in bad.items():
                dropped[key] = f"value rejected: {why}"
                preserved.pop(key, None)

    return preserved, dropped, None


def _backup_unreadable_config(path: Path) -> Path:
    """Copy a config we could not parse aside before it is overwritten.

    Mirrors ``cli/bootstrap_kismet.backup_kismet_site_conf``'s shape — a
    timestamp-suffixed sibling — rather than inventing a second convention.
    Byte-for-byte via ``shutil.copy2``: the file did not parse, so re-serialising
    it is not an option, and its unparseable form is exactly what the operator
    needs in order to see what went wrong.

    Never overwrites an existing backup; if one is already there, keeps counting.
    Raises ``OSError`` when it cannot write, which the caller turns into a refusal.
    """
    ...
    while candidate.exists():
        candidate = path.with_name(f"{path.name}.unreadable-{stamp}.{n}")
        n += 1
...
# Rules below are NOT delegation rules: their `patterns:` lists name real
# decoded values (not a sentinel for "consult the watchlist DB"), and the
# `severity` field below is the one that fires at alert time, not a
# placeholder for the matched watchlist row's severity column. The
# ble_device_class rule, in particular, decodes the class from the BLE
# advertisement itself (see lynceus/ble_continuity.py); the Kismet
# classic-HCI path surfaces no payload to decode, so the rule is inert
# unless the passive BLE bridge is on. The wizard derives the rule's
# active/commented state from the operator's ble_bridge.enabled answer
# rather than adding a new prompt -- nothing but the bridge ever
# populates the decoded class column, and enabling the rule alongside
# the bridge is the only configuration that does what the operator
# asked for.
#
# Tuple shape mirrors DELEGATION_RULES: (name, rule_type, severity,
# patterns_inline, description). patterns_inline is rendered as a
# YAML inline list rather than the delegation idiom `patterns: []`.
DECODED_CLASS_RULES: tuple[tuple[str, str, str, str, str], ...] = (
    (
        "apple_find_my",
        "ble_device_class",
        "med",
...
def render_rules_yaml(
    ...
    *,
    ...
    The decoded-class section (currently just ``apple_find_my``) is
    appended after the delegation entries. The rule is ACTIVE when
    ``ble_bridge_enabled`` is True and commented-out otherwise — the
    bridge is the only path that populates ``ble_device_class``, so the
    rule cannot fire without it; turning it on alongside the bridge is
    the only configuration that does what the operator asked for. The
    loud call-out above the block tells the operator plainly which is
    which and how to flip it.
    """
    lines = [
    ...
    # ---- Decoded BLE device class section --------------------------------
    # The rule below is NOT delegation: patterns are real, severity is the
    # one that fires. Loud call-out so an operator reading the file knows
    # which case they got, why, and how to change it.
    lines.append("")
    lines.extend(
        [
            "  # ---- Apple Find My (decoded BLE device class) ----",
            "  # Alerts on Apple Find My trackers detected away from their owner.",
            "  # Unlike the watchlist_* entries above this is NOT delegated: the",
            "  # class is decoded from the BLE advertisement payload itself, so",
            "  # the patterns below are real and the `severity` field is the one",
            "  # that fires — not a watchlist DB lookup.",
            "  #",
            (
                "  # ACTIVE because the BLE bridge (ble_bridge.enabled in"
                if ble_bridge_enabled
                else "  # Currently COMMENTED because the BLE bridge (ble_bridge.enabled in"
            ),
            "  # lynceus.yaml) is on. Nothing else populates `ble_device_class` —"
            if ble_bridge_enabled
            else "  # lynceus.yaml) is off. Nothing else populates `ble_device_class` —",
            "  # the Kismet classic-HCI path surfaces no advertisement payload to",
            "  # decode — so this rule cannot fire without the bridge.",
            "  #",
            (
                "  # To disable: comment the block back out, or set"
                if ble_bridge_enabled
                else "  # To enable: set ble_bridge.enabled to true in lynceus.yaml AND"
            ),
            (
                "  # ble_bridge.enabled to false in lynceus.yaml."
                if ble_bridge_enabled
                else "  # uncomment the block below."
            ),
        ]
    )
    prefix = "  " if ble_bridge_enabled else "  # "
    for index, (name, rule_type, severity, patterns_inline, description) in enumerate(
        DECODED_CLASS_RULES
    ):
        if index > 0:
            lines.append("")
        lines.extend(
            [
                ...
                f"{prefix}  patterns: [{patterns_inline}]",
                f'{prefix}  description: "{description}"',
            ]
        )
    lines.append("")
...
def apply_config(
    ...
    *,
    ...
    preserved, dropped_keys, preserve_error = carry_forward_settings(target_path, content)
    content = content + render_carried_forward_block(preserved)
    # ⛔ If we could not READ the previous config, we must not silently destroy
    # it. Reporting a warning while `write_config` overwrites the file anyway is
    # fail-OPEN with respect to the operator's data: one YAML typo, or a
    # transient permission problem, and every hand-edited setting — including
    # `ntfy_auth_token` and any other secret in there — is gone, with the only
    # notice arriving after the bytes have already been replaced.
    #
    # Refusing outright would be wrong too: re-running setup over a config that
    # no longer parses is a legitimate, likely recovery move, and blocking it
    # strands the operator. So: keep a copy, then proceed. If even the copy
    # cannot be made, THEN refuse — at that point we would be destroying data we
    # cannot give back.
    backup_path: Path | None = None
    if preserve_error is not None and target_path.exists():
        try:
            backup_path = _backup_unreadable_config(target_path)
        except OSError as exc:
            raise SetupError(
                f"Refusing to overwrite {target_path}: it could not be read "
                f"({preserve_error}) and a backup could not be written either "
                f"({exc}). Overwriting now would destroy settings that cannot "
                f"be recovered. Move the file aside by hand and re-run."
            ) from exc
    ...
    _emit(
        ApplyStep(
            name="write_config",
            status="warning" if (preserve_error or dropped_keys) else "ok",
            message=(
                f"Config written to {target_path}. The previous config could not "
                f"be read ({preserve_error}), so its settings were NOT carried "
                f"forward — a byte-for-byte copy is at {backup_path}"
                if preserve_error
                else (
                    ...
                    + (
                        f". DROPPED {len(dropped_keys)} setting"
                        f"{'' if len(dropped_keys) == 1 else 's'} that would have "
                        f"stopped the daemon loading this file at all: "
                        + "; ".join(f"{k} ({why})" for k, why in sorted(dropped_keys.items()))
                        if dropped_keys
                        else ""
                    )
                )
            ...
            detail={
                ...
                "dropped": dict(sorted(dropped_keys.items())),
    ...
    # Gated on Config.rules_path set AND (delegation types selected OR
    # the BLE bridge is enabled). The CLI frontend in v0.6.x runs its
    # alerting prompts AFTER apply_config returns (so per-type counts
    # reflect the just-imported DB), so this branch is only exercised
    # by callers that pre-decide alerting — the eventual web wizard,
    # and Touch 2's new tests.
    #
    # The bridge leg of the OR is new (Packet B). Without it, an operator
    # who enables the passive BLE bridge but answers N at every
    # delegation prompt ends up with ``enabled_rule_types == set()`` and
    # ``config.rules_path`` still wired in — and the Find My rule,
    # which is derived from the bridge answer rather than asked for in
    ...
    if rules_target is not None and writing_for_bridge and rules_target.exists():
        ...
        # `active` powers the operator-facing count + the `enabled`
        # detail list. Include the Find My rule_type when the bridge is
        # on so both numbers reflect what the daemon will actually see
        ...
        if not config.rules_path:
            reason = (
                "enabled_rule_types or BLE bridge enabled but Config.rules_path not set"
                if (enabled_rule_types or config.ble_bridge.enabled)
                else "Alerting not enabled by caller"
            )
        else:
            reason = "Config.rules_path set but neither delegation types nor BLE bridge enabled"
        _emit(
            ApplyStep(
                name="write_rules",
                status="skipped",
